...
interface VaultCardProps {
  ...
  isSyncing?: boolean;
  metadata?: VaultOperationalMetadata;
  onConnect: (id: string, keyValues: Record<string, string>) => Promise<void>;
}

const VaultCard: React.FC<VaultCardProps> = ({ integration, isAuthorized, isSyncing, metadata, onConnect }) => {
      ...
      <div className="relative p-5">
        ...
        {isAuthorized && metadata && (
          <div
            ...
            className="mt-4 rounded-xl border border-emerald-500/10 bg-black/40 p-3 text-[11px] text-[var(--t3)] space-y-2"
          >
            <div className="flex items-center justify-between gap-3">
              <span className="uppercase tracking-widest font-semibold text-[var(--t4)]">Operational metadata</span>
              <span className={`px-2 py-0.5 rounded-full border uppercase tracking-wider text-[10px] ${
            ...
            <div className="grid grid-cols-1 gap-1.5 font-mono">
              <div><span className="text-[var(--t4)]">Auth scheme:</span> {metadata.authScheme || 'Bearer'}</div>
              {metadata.login && <div><span className="text-[var(--t4)]">Authenticated as:</span> {metadata.login}</div>}
              <div><span className="text-[var(--t4)]">Visible repos:</span> {metadata.visibleRepos?.length ? metadata.visibleRepos.join(', ') : 'none verified'}</div>
              {metadata.targetRepo && <div><span className="text-[var(--t4)]">Target repo:</span> {metadata.targetRepo}</div>}
              {metadata.lastVerified && <div><span className="text-[var(--t4)]">Last verified:</span> {new Date(metadata.lastVerified).toLocaleString()}</div>}
              {metadata.lastFailure && <div className="text-amber-300/90"><span className="text-[var(--t4)]">Last failure:</span> {metadata.lastFailure}</div>}
            </div>
          </div>
        )}
