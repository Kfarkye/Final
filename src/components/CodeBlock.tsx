  ...
  return (
    <div className="relative group my-4">
      <div className="absolute right-2 top-2 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
        {lang && <span className="text-[10px] uppercase tracking-wider text-white/40 px-1 select-none">{lang}</span>}
        <button onClick={copy} className="text-[11px] font-medium px-2 py-1 rounded-md bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition-colors">
          {copied ? '✓ Copied' : 'Copy'}
        </button>
        <DownloadMenu content={code} fileName={`truth-export.${ext}`} mime={mime} tone="code" />
      </div>
      <pre className="bg-white/5 p-4 rounded-xl overflow-x-auto text-[13px] font-mono">{children}</pre>
    </div>
  );
};
